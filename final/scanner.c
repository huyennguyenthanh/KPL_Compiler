...
// a keyword may be include char and digit
  ...
  {
      token -> string[len] = toupper((char)currentChar);
      len++;
      readChar();
  }

  if (len > MAX_IDENT_LEN) {
    error(ERR_IDENT_TOO_LONG, lineNo, colNo - len);
...
void printToken(Token *token) {
  ...
  case SB_DEC: printf("SB_DEC\n"); break;
  case SB_INC: printf("SB_INC\n"); break;
